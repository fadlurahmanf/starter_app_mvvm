...
class LoggerViewModel @Inject constructor(
    ...
    private val _state = MutableLiveData<CustomState<List<String>>>()
    val state: LiveData<CustomState<List<String>>> = _state
    fun getAllHistory() {
        _state.value = CustomState.Idle
        _state.value = CustomState.Loading

        disposable().add(loggerInteractor.getAll().subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread()).subscribe({
                _state.value = CustomState.Success(data = it)
            }, {
                logConsole.e("MASUK SINII ${it.message}")
